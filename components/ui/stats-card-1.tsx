...
export const StatsCard = React.forwardRef<HTMLDivElement, StatsCardProps>(
  ({ title, currentValue, valuePrefix, valuePostfix, description, chartData, onActionClick, className, defaultBarColor = "bg-primary/20", highlightedBarColor = "bg-destructive" }, ref) => {
    ...
    const barVariants = {
      hidden: { height: "0%" },
      visible: { height: "var(--bar-height, 0%)", transition: { type: "spring" as const, damping: 15, stiffness: 100 } },
    };
      ...
      >
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          <HeaderElement
            ...
            className={cn("flex w-full items-center justify-between text-left", onActionClick && "group rounded-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring")}
          >
            <h3 className="text-sm font-medium text-muted-foreground">{title}</h3>
            {onActionClick && <ChevronRight className="h-4 w-4 text-muted-foreground transition-transform group-hover:translate-x-1" />}
          </HeaderElement>
        </CardHeader>
        <CardContent>
          <div ref={cardRef} className="flex flex-col gap-4">
            ...
            <motion.div className="flex h-20 w-full items-end gap-3" initial="hidden" animate={controls} transition={{ staggerChildren: 0.1 }}>
              {chartData.map((item, index) => (
                <div key={item.name} className="flex h-full flex-1 flex-col items-center justify-end gap-1">
                  <motion.div
                    className={cn("w-full rounded-t-lg", item.color ? item.color : index === chartData.length - 1 ? highlightedBarColor : defaultBarColor)}
