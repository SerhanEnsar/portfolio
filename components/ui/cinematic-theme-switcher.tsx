...
export default function CinematicThemeSwitcher() {
  ...
  const handleToggle = () => { generateParticles(); setTheme(isDark ? 'light' : 'dark'); };
    ...
    <div className="relative inline-block">
      <svg className="absolute w-0 h-0">
        <defs>
          <filter id="grain-light">
            ...
            <feComponentTransfer in="desaturatedNoise" result="lightGrain"><feFuncA type="linear" slope="0.3" /></feComponentTransfer>
            <feBlend in="SourceGraphic" in2="lightGrain" mode="overlay" />
          </filter>
          <filter id="grain-dark">
            <feTurbulence type="fractalNoise" baseFrequency="0.9" numOctaves="4" result="noise" />
            <feColorMatrix in="noise" type="saturate" values="0" result="desaturatedNoise" />
            <feComponentTransfer in="desaturatedNoise" result="darkGrain"><feFuncA type="linear" slope="0.5" /></feComponentTransfer>
      ...
      <motion.button
        ...
        style={{
          ...
          boxShadow: isDark
            ? 'inset 5px 5px 12px rgba(0,0,0,0.9), inset -5px -5px 12px rgba(71,85,105,0.4), 0 8px 16px rgba(0,0,0,0.4)'
            : 'inset 5px 5px 12px rgba(148,163,184,0.5), inset -5px -5px 12px rgba(255,255,255,1), 0 8px 16px rgba(0,0,0,0.08)',
          ...
          {isAnimating && particles.map((particle) => (
            <motion.div key={particle.id} className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <motion.div
                ...
                style={{ width: '10px', height: '10px', background: isDark ? 'radial-gradient(circle, rgba(147,197,253,0.5) 0%, rgba(147,197,253,0) 70%)' : 'radial-gradient(circle, rgba(251,191,36,0.7) 0%, rgba(251,191,36,0) 70%)' }}
